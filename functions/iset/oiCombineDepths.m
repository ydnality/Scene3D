...
%Andy: not sure if we still need this code... commented out
% % Allocate space for
sz = oiGet(oiD{1},'size');
...
for ii=nDepths:-1:1
    ...
    p(isnan(p)) = 0;
    %borrowed from sceneDepthRange code... too small to make into separate
    %function? perhaps consider doing this
    
    %dPlane = (depthEdges(ii) <= dMap) & (dMap < depthEdges(ii+1));
    
    %identify hole regions
    %hRegions = 1-dPlane;
    
    %fill hole regions of the dPlane pixels
    ...
    %feather the mask by using blur of depthPlane
%     photons = p .* repmat(dMaps(:,:,ii) * 1.0 , [ 1 1 31]) + photons;
    %alphaMask{ii} = alphaMask{ii}.^.9;
  
    if (ii < nDepths)
        photons = p .* repmat(alphaMask{ii},[1 1 31]) + photons .* repmat(1 - alphaMask{ii},[1 1 31]) ;
 %photons = p+ photons ;
...
% Professor Wandell wrote the following code, but he commented it out.  Left
